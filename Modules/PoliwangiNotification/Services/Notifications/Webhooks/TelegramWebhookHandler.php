...
namespace Modules\PoliwangiNotification\Services\Notifications\Webhooks;

use App\Conversation;
use Modules\PoliwangiNotification\Services\Notifications\NotificationWebhookInterface;
...
class TelegramWebhookHandler implements NotificationWebhookInterface
{
    ...
    private function applyAction(
        ...
        switch ($action) {
            case 'active':
                $newStatus = Conversation::STATUS_ACTIVE;
                $statusText = 'Aktif';
                break;

            case 'pending':
                $newStatus = Conversation::STATUS_PENDING;
                $statusText = 'Pending';
                break;
            ...
            case 'spam':
                $newStatus = Conversation::STATUS_SPAM;
                $statusText = 'Spam';
                break;
        ...
        // Cari user untuk webhook action (Assigned user atau Admin)
        $user = null;
        ...
        if ($user && method_exists($conversation, 'changeStatus')) {
